...
void addVBOHeader( void ) {
	...
	if( fileVBO ) {
		...
		fileVBO.println( tempStr );
		fileVBO.println("\n[column names]");
		fileVBO.println("sats lat long velocity heading height longacc latacc device-update-rate lean-angle fix-type coordinate-precision altitude-precision sats time lat long velocity heading height longacc latacc x-acceleration-datalogger y-acceleration-datalogger z-acceleration-datalogger x-rotation-datalogger y-rotation-datalogger z-rotation-datalogger device-update-rate-datalogger rpm-datalogger roll-datalogger pitch-datalogger yaw-datalogger tps-datalogger neutral-datalogger brake-switch-datalogger gear-raw-datalogger gear-num-datalogger front-brake-pressure-datalogger front-suspension-datalogger front-rpm-datalogger rear-suspension-datalogger rear-rpm-datalogger temp-datalogger pressure-datalogger");
		fileVBO.println("\n[data]");
	}
}
...
void populateVBOLine() {
	...
	gps.satellites.value(),		// satellites
	hour(),	minute(), gps.time.second(), ((gps.time.centisecond()*10) + gps.time.age()),		// time
	gps.location.lat(),			// latitude
	gps.location.lng(),			// longitude
	gps.speed.kmph(),			// velocity kmh
	gps.course.deg(),			// heading
